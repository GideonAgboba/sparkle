    		...
    		$output = NULL;
              if (isset($_POST["search_tex_ext"])) {
                $search_tex_ext = $_POST["search_tex_ext"];
			$queryelm = "SELECT * FROM texcote_exterior WHERE title = '$search_tex_ext'";
			...
			?>
				<div class="white container p-5 justify-content-center my-auto" style="border-top: 10px solid #fff; border-bottom: 5px solid #25A187; border-left: 5px solid #25A187; border-right: 5px solid #25A187; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
					<form method="POST" action="texcote-exterior.php?action=add&id=<?php echo $row["id"]; ?>">
  ...
  if (confirm("Are you done shopping?")) {
    ...
    window.location = 'index.php';
      ...
      <nav class="navbar-expand white fixed-top">
      	...
      	<form style="width: 400px;" class="center brand-logo" method="POST" action="texcote-exterior.php">
      	<input type="text" name="search_tex_ext" class="form-control validate text-center" placeholder="search item" value="<?php if(isset($_POST['search_tex_ext'])){ echo $_POST['search_tex_ext'];} ?>">
      		<?php echo $output; ?>
      	</form>
	...
	<div class="container-fluid green-body green-font" style="padding-top: 5em;">
		...
		<?php
			$query = "SELECT * FROM texcote_exterior ORDER BY id ASC";
			$result = mysqli_query($connect, $query);
			if (mysqli_num_rows($result) >0) {

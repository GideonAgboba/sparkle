    		...
    		$output = NULL;
              if (isset($_POST["search_gloss"])) {
                $search_gloss = $_POST["search_gloss"];
			$queryelm = "SELECT * FROM gloss WHERE title = '$search_gloss'";
			...
			?>
				<div class="white container p-5 justify-content-center my-auto" style="border-top: 10px solid #fff; border-bottom: 5px solid #25A187; border-left: 5px solid #25A187; border-right: 5px solid #25A187; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
					<form method="POST" action="gloss.php?action=add&id=<?php echo $row["id"]; ?>">
  ...
  if (confirm("Are you done shopping?")) {
    ...
    window.location = 'index.php';
      ...
      <nav class="navbar-expand white fixed-top">
      	...
      	<form style="width: 400px;" class="center brand-logo" method="POST" action="gloss.php">
      	<input type="text" name="search_gloss" class="form-control validate text-center" placeholder="search item" value="<?php if(isset($_POST['search_gloss'])){ echo $_POST['search_gloss'];} ?>">
      		<?php echo $output; ?>
      	</form>
